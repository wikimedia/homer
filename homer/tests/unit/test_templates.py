...
from homer.templates import JinjaRenderer
from homer.tests import get_fixture_path


class TestJinjaRenderer:
    """Jinja render class tests."""

    def setup_method(self):
        """Initialize the test instances."""
        # pylint: disable=attribute-defined-outside-init
        self.renderer = JinjaRenderer(get_fixture_path('templates'), '')

    def test_render_ok(self):
        """Should return the rendered template."""
        assert str(self.renderer.render('valid', {'test_key': 'test_value'}, [])) == 'test_value;'
